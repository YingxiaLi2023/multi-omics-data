...
evaluatesetting <- function(iter) {
  ...
  for(i in seq(along=block)) {
    if(length(block[[i]]) <= 1000) {
      ...
      inds <- which(colnames(Xtrain) %in% colnames( Xtrain_s[,order(varimp,decreasing = T)[1:1000]]))
  ...
  save(res, file=paste("./Features_1000/Results/Results_3_1000/rda_files/res", iter, ".Rda", sep=""))
