...
# Set the working directory to the directory 'multi-omics-data-main' 
# of the electronic appendix (outcomment the following line
# and replace 'pathtomulti-omics-data' by the path to 'multi-omics-data-main'
# on your computer):

## setwd("pathtomulti-omics-data/multi-omics-data-main")
...
evaluatesetting <- function(iter) {
  ...
  if(comb=="rna") {
    ...
    X <- cbind(clindata, rnadata)
    
    block <- rep(1:2, times=c(ncol(clindata), ncol(rnadata)))
    ...
    X <- cbind(clindata, mirnadata)
    
    block <- rep(1:2, times=c(ncol(clindata),ncol(mirnadata)))
    ...
    X <- cbind(clindata, methydata)
    
    block <- rep(1:2, times=c(ncol(clindata), ncol(methydata)))
    
    block <- lapply(1:2, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="mutation"){
    
    X <- cbind(clindata, mutationdata)
    
    block <- rep(1:2, times=c(ncol(clindata), ncol(mutationdata)))
    ...
    X <- cbind(clindata, cnvdata)
    
    block <- rep(1:2, times=c(ncol(clindata), ncol(cnvdata)))
    
    block <- lapply(1:2, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_mirna_methy_mutation"){
    
    X <- cbind(clindata,rnadata, mirnadata, methydata,mutationdata)
    
    block <- rep(1:5, times=c(ncol(clindata), ncol(rnadata), ncol(mirnadata), ncol(methydata), ncol(mutationdata)))
    
    block <- lapply(1:5, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_mirna_methy_cnv"){     
    
    X <- cbind(clindata, rnadata, mirnadata, methydata, cnvdata)
    
    block <- rep(1:5, times=c(ncol(clindata), ncol(rnadata), ncol(mirnadata), ncol(methydata),ncol(cnvdata)))
    
    block <- lapply(1:5, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_mirna_mutation_cnv"){
    
    X <- cbind(clindata, rnadata, mirnadata, mutationdata, cnvdata)
    
    block <- rep(1:5, times=c(ncol(clindata), ncol(rnadata), ncol(mirnadata), ncol(mutationdata), ncol(cnvdata)))
    
    block <- lapply(1:5, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_methy_mutation_cnv"){
    
    X <- cbind(clindata, rnadata, methydata, mutationdata, cnvdata)
    
    block <- rep(1:5, times=c(ncol(clindata), ncol(rnadata), ncol(methydata), ncol(mutationdata), ncol(cnvdata)))
    
    block <- lapply(1:5, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="mirna_methy_mutation_cnv"){
    
    X <- cbind(clindata, mirnadata, methydata, mutationdata, cnvdata)
    
    block <- rep(1:5, times=c(ncol(clindata), ncol(mirnadata), ncol(methydata), ncol(mutationdata), ncol(cnvdata)))
    
    block <- lapply(1:5, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_mirna_methy_mutation_cnv"){
    
    X <- cbind(clindata, rnadata, mirnadata, methydata, mutationdata, cnvdata)
    
    block <- rep(1:6, times=c(ncol(clindata), ncol(rnadata), ncol(mirnadata), ncol(methydata), ncol(mutationdata), ncol(cnvdata)))
    
    block <- lapply(1:6, function(x) which(block==x))
  ...
  # Perform supervised variable selection per block using ranger
  indsel <- c()
  blocksub <- c()
  
  for(i in seq(along=block)) {
    if(length(block[[i]]) <=1000) {
      ...
      Xtrain_snz <- Xtrain_s[, apply(Xtrain_s, 2, sum) != 0]
      dataall <- Xtrain_snz
      dataall$survivaltime <- ytrain[,1]
      dataall$statusvariable <- ytrain[,2]
      varimp <- ranger(data=dataall, dependent.variable.name = "survivaltime", status.variable.name = "statusvariable", 
                        importance="permutation", splitrule="extratrees", num.trees=10000, num.threads=1)$variable.importance
      inds <- which(colnames(Xtrain) %in% colnames( Xtrain_snz[,order(varimp,decreasing = T)[1:1000]]))
      indsel <- c(indsel, inds)
      blocksub <- c(blocksub, rep(i, 1000))
...
##reduce memory
  ...
  save(res, file=paste("./Features_1000/Results/Results_1_4_5_1000/rda_files/res", iter, ".Rda", sep=""))
...
#pec_prioritylasso, the code comes from https://github.com/HerrMo/multi-omics_benchmark_study/blob/master/R/ancillary_code_bench.R 
...
prioritylassowrap <- function(Xtrain, ytrain, Xtest, ytest, blocksub) {
  ...
  # calculate ibreir
  mod <- cvres
  coeffs <- mod$coeff # -1 to remove intercept (which is NA for Cox!)
  index_nz = get_nz(coeffs)
  cph <- compute_coxph_mod(coeffs, index_nz, traindata)
  mod$cph <- cph
  mod$index_nz <- index_nz
