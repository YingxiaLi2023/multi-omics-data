...
evaluatesetting <- function(iter) {
  ...
  if(comb=="rna_mirna") {
    ...
    X <- cbind(clindata, rnadata, mirnadata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(rnadata),ncol(mirnadata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_methy"){
    
    X <- cbind(clindata, rnadata, methydata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(rnadata),ncol(methydata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_mutation"){
    X <- cbind(clindata, rnadata, mutationdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(rnadata),ncol(mutationdata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="rna_cnv"){
    
    X <- cbind(clindata, rnadata, cnvdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(rnadata),ncol(cnvdata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="mirna_methy"){
    
    X <- cbind(clindata, mirnadata, methydata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(mirnadata),ncol(methydata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="mirna_mutation"){     
    
    X <- cbind(clindata, mirnadata, mutationdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(mirnadata),ncol(mutationdata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="miran_cnv"){
    
    X <- cbind(clindata, mirnadata, cnvdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(mirnadata),ncol(cnvdata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="methy_mutation"){
    
    X <- cbind(clindata, methydata, mutationdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(methydata),ncol(mutationdata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="methy_cnv"){
    
    X <- cbind(clindata, methydata, cnvdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(methydata),ncol(cnvdata)))
    
    block <- lapply(1:3, function(x) which(block==x))
    
    y <- cbind(surdata$time, surdata$status)
    
  } else if(comb=="mutation_cnv"){
    
    X <- cbind(clindata, mutationdata, cnvdata)
    
    block <- rep(1:3, times=c(ncol(clindata), ncol(mutationdata),ncol(cnvdata)))
  ...
  save(res, file=paste("./Features_1000/Results/Results_2_1000/rda_files/res", iter, ".Rda", sep=""))
